    ...
    Thread {
        ...
        try {
            do {
                ...
                // 确认其回到就绪(@)状态后再继续下一张，避免缓冲粘连导致重叠/报错
                // （与 USB 模式一致：发完 -> sleep(300) -> 读状态确认 -> 继续）。
                Thread.sleep(300)
 ...
 * 实现流程（对齐 USB 的 getPrinterReadyUsb）：调用方已先 sleep(300) 让打印机消化，
 * 本函数发命令 → 读回传包 → 必须解析到明确的就绪字符 '@'(0x40) 才返回；
 * 读到忙(P/B/C/W)/暂停(`)/其它则继续重发查询；完全读不到则 [timeoutMillis] 兜底放行，避免批量卡死。
 *
