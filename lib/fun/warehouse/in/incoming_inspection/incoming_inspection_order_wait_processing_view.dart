...
class OrderWaitProcessingPage extends StatefulWidget {
  const OrderWaitProcessingPage({super.key});

  @override
  State<OrderWaitProcessingPage> createState() =>
      _OrderWaitProcessingPageState();
}

class _OrderWaitProcessingPageState extends State<OrderWaitProcessingPage> {
            ...
            children: [
              Expanded(
                child: CombinationButton(
                  ...
                  text: 'incoming_inspection_order_sign'.tr,
                  click: () => askDialog(
                    content: 'incoming_inspection_order_sign_tips'.tr,
                    confirm: () => logic.signOrder(),
                  ),
                ),
              ),
              Expanded(
                child: CombinationButton(
                  ...
                  text: 'incoming_inspection_order_exception'.tr,
                  click: () => reasonInputPopup(
                    title: [
                      Center(
                        child: Text(
                          'incoming_inspection_order_exception'.tr,
                          style: const TextStyle(
                            fontSize: 20,
                            color: Colors.white,
                          ),
                    ...
                    hintText:
                        'incoming_inspection_order_input_exception_tips'.tr,
                    confirmText:
                        'incoming_inspection_order_report_exception'.tr,
                    isCanCancel: true,
                    confirm: (s) {
                      Get.back();
                      logic.reportException(exceptionInfo: s);
                    },
                  ),
                ),
              ),
