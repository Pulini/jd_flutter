...
class OrderExceptionHandlingPage extends StatefulWidget {
  const OrderExceptionHandlingPage({super.key});

  @override
  State<OrderExceptionHandlingPage> createState() =>
      _OrderExceptionHandlingPageState();
}

class _OrderExceptionHandlingPageState
    extends State<OrderExceptionHandlingPage> {
  ...
  Widget build(BuildContext context) {
    return pageBody(
      title: 'incoming_inspection_order_exception_dispose'.tr,
      body: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          SizedBox(
          ...
          SizedBox(
            ...
            child: CombinationButton(text: 'incoming_inspection_order_dispose'.tr, click: () =>reasonInputPopup(
              title: [
                 Center(
                  child: Text(
                    'incoming_inspection_order_dispose'.tr,
                    style: const TextStyle(fontSize: 20, color: Colors.white),
                  ),
                )
              ],
              hintText: 'incoming_inspection_order_input_exception_result'.tr,
              confirmText: 'incoming_inspection_order_submit_processing_result'.tr,
              isCanCancel: true,
              confirm: (s) {
                Get.back();
                logic.exceptionHandling(handlingResult: s);
              },
            ),),
          ),
        ],
