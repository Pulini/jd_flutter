...
class _WarehouseShareReportPageState extends State<WarehouseShareReportPage> {
  ...
  final WarehouseAllocationState state = Get.find<WarehouseAllocationLogic>().state;
          ...
          SizedBox(
            ...
            child: DecoratedBox(
              decoration: const BoxDecoration(
                  border: Border(left: BorderSide(width: 1,color: Colors.black),right: BorderSide(width: 1,color: Colors.black))),
              child: Center(child: Text(value.content.toString(),style: TextStyle(color: Color(int.parse("0xFF${value.foreColor.toString().replaceAll("#", "")}")),),))),
          )
      ],
    );
  }
      ...
      body: Obx(
            ...
            ListView.builder(
              ...
              itemBuilder: (context, index) =>
                  Container(
                    padding:
                    const EdgeInsets.only(left: 5, top: 3, right: 3, bottom: 3),
                    decoration: BoxDecoration(
                        color: Color(int.parse("0xFF${state.reportDataList[index].backgroundColor.toString().replaceAll("#", "")}")),
