  ...
  Widget build(BuildContext context) {
    return pageBody(
      ...
      body: Obx(() => ListView.builder(
            ...
            itemBuilder: (c, i) => _LastProcessReportItem(
              data: state.lastProcessGroupPayList[i],
              onTap: () => logic.getLastProcessDetails(
                  state.lastProcessGroupPayList[i].groupPayInterID ?? -1),
              onDelete: () => logic.deleteLastProcessReportOrder(
                  id: state.lastProcessGroupPayList[i].groupPayInterID ?? -1),
            ),
          )),
    );
  }
...
class _LastProcessReportItem extends StatelessWidget {
  final LastProcessGroupPayInfo data;
  final VoidCallback onTap;
  final VoidCallback onDelete;

  const _LastProcessReportItem({
    required this.data,
    required this.onTap,
